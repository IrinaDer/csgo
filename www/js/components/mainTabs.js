...
var rotatorItemHTMLgenerate = function (item, winner) {
    ...
    blockHTML += '<div title="' + item.name + ' ' + item.subname + '" data-id="' + item.id + '" data-points="'+ item.points +'" class="pcase__preitem ' + item.color + winnerClass +'">';
...
var rotatorItemsGenerate = function () {
    var rotatorArr = [], // масив предметов для ротатора (пока пустой)
        ...
        winnerID = randomInt(1, 11), // победный id
...
var rotatorGoToWinner = function () {
    ...
    $(document).on('click', '#fastedCase', function(event) {
    ...
    winHTML += '<div class="pcase__rotor__winitem ' + vinnerObj.color + '" data-points="'+ vinnerObj.points +'">';
    winHTML +=     '<div class="pcase__preitem__top-text">StatTrack <span>ТМ</span></div>';
    ...
    winHTML +=     '<div class="pcase__preitem__wear"><span>Minimal Wear</span></div>';
...
$(document).on('click', '#openCase',function(event) {
    ...
    if (balance >= price) {
        if (isOpenedCase) {
            ...
            var points = $(document).find('.pcase__rotor__winitem').data('points');
            window.custom.levelPlus(+points);
        }
...
$(document).on('click', '#soldWinItem', function(event) {
    $('.pcase__itsincase__items').find('.pcase__itsincase__item').removeClass('active');
    isOpenedCase = false;
    var price = parseFloat($(this).attr('data-price'));
